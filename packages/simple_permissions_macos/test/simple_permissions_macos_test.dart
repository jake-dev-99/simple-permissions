...
import 'package:simple_permissions_macos/simple_permissions_macos.dart';
import 'package:simple_permissions_macos/src/macos_permission_registry.dart';
import 'package:simple_permissions_macos/src/permissions_macos_api.dart';
...
class MockPermissionsMacosApi implements PermissionsMacosApi {
...
void main() {
  late SimplePermissionsMacos plugin;
  late MockPermissionsMacosApi mockApi;

  setUp(() {
    mockApi = MockPermissionsMacosApi();
    plugin = SimplePermissionsMacos(api: mockApi);
  });
  ...
  group('SimplePermissionsMacos', () {
    ...
    test('registerWith sets platform instance', () {
      SimplePermissionsMacos.registerWith();
      expect(
        SimplePermissionsPlatform.instance,
        isA<SimplePermissionsMacos>(),
      );
    });
  });
  ...
  group('check()', () {
    ...
    test('routes coarse location through API', () async {
      mockApi.checkResult = 'granted';
      final result = await plugin.check(const CoarseLocation());

      expect(result, PermissionGrant.granted);
      expect(mockApi.log.first.identifier, 'coarse_location');
    });
    ...
    test(
      ...
      for (final perm in mobileOnly) {
        final result = await plugin.check(perm);
        expect(
          result,
          PermissionGrant.notApplicable,
          reason: '${perm.runtimeType} should be notApplicable on macOS',
        );
      }

      expect(mockApi.log, isEmpty,
          reason: 'No API calls should be made for mobile-only permissions');
    });
  });
  ...
  group('VersionedPermission resolution on macOS', () {
    ...
    test(
      ...
      // ReadMediaAudio has no macOS handler → notApplicable, no API call.
